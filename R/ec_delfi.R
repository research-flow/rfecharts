...
ec_delfi <- function(data, colors = NULL, show_positives = "inclusive", nc = RFecharts::name_cleaner(), co = RFecharts::color(), as_json = shiny::isRunning(), theme = RFecharts::rf_echarts_theme) {
  ...
  if (show_positives == "inclusive" | show_positives == "exclusive") {
    ...
    e_show_positives <- function(e) {
      e_mark_line(e,
        data = list(xAxis = 1 - rate_positives),
        precision = 5,
        silent = TRUE,
  ...
  data |>
    ...
    function(params){
      return (params.value[0] * 100).toFixed(0) + '%';
    }
  ")
      ),
      itemStdatale = list(
    ...
    function(params) {
      ...
      params.forEach(function(p) {
        let percent = (p.value[0] * 100).toFixed(1) + '%';
        let count = p.data.bind;
        output += p.marker + p.seriesName + ': ' + p.name + '<br/>';
      });
      return output;
    }
  ")
